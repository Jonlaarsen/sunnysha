  ...
  try {
    ...
    if (!user) {
      return NextResponse.json(
        { 
          error: "Unauthorized", 
          message: "You must be logged in to update records" 
        },
        { status: 401 }
      );
    }

    const body: UpdateFormData = await req.json();
    ...
    if (error) {
      ...
      return NextResponse.json(
        {
          error: "Failed to update QC record in database",
    ...
    console.log("Successfully updated QC record in Supabase:", data);
    ...
    console.error("Error updating QC form data:", normalizedError);

    return NextResponse.json(
      {
        error: "Failed to update QC form data",
